...
#### MAIN LOOP OVER DATASETS:
    ...
    for basename in datanames: # Loop over datasets
        ...
        import dloader_v1
       # if 'Categorical' not in D.feat_type:
       #     print('no categorical')
       #     Train_data = D.data['X_train']
       #     Test_data = D.data['X_test']
       #     Vali_data = D.data['X_valid']
       # else:

        Data_load = dloader_v1.GCDataLoader_v1()
        Data_load.load(D.data['X_train'],False,feat_type = D.feat_type)
        Train_data = Data_load.data

        Test_load = Data_load.load(D.data['X_test'],False,feat_type = D.feat_type)
        Test_data = Test_load.data

        Vali_load = Data_load.load(D.data['X_valid'],False,feat_type = D.feat_type)
        Vali_data = Vali_load.data

        M = autosklearn.classification.AutoSklearnClassifier(per_run_time_limit=int(time_budget)-100,\
                        time_left_for_this_task=int(time_budget)-100,ml_memory_limit=8000)
        M.fit(Train_data, D.data['Y_train'],metric=autosklearn.metrics.roc_auc)
        ...
        # Make predictions
        # -----------------
        Y_valid = M.predict_proba(Vali_data)[:,1]
        Y_test = M.predict_proba(Test_data)[:,1]
